...
def some_function():
    ...
    print_pause(f"The enemy {enemy} ...... emerges swiftly.")
    if enemy == "Zamza":
        print_pause(f"Zamza is very powerful, but luckily you are equiped to "
                    "deal with his barage of attacks.  You not only see him, "
                    " your Broadsword is powerful enough to defeat him")
    elif enemy == "Vehelits":
...
def valid_input(prompt, option1, option2, option3):
    while True:
        ...
        if option1 in character:
            break
        elif option2 in character:
            break
        elif option3 in character:
            break
        else:
            print_pause("Sorry, not a valid input.")
    return character
...
def choose_character():
        character = valid_input("Please choose your character. "
                                "Axel Blaze or Adam: ",
                                "Axel", "Blaze", "Adam:").lower()

        if "Axel" in character:
             print_pause("Thank you for your choice")
        elif "Blaze" in character:
            print_pause("Thank you for your choice")
        elif "Adam" in character:
            print_pause("Thank you for your choice")



            print_pause(f"You have chosen {character}, a great warrior indeed")

            print_pause(f"{character}, you have landed on the Streets of Rage.\n ")


def street_choice():
    print_pause("Please choose the name of the street you would "
                "like travel to.")
...
def east_street():
    ...
    if "Broadsword" in items:
        ...
        print_pause("You ask, 'which weapon will serve me well "
...
def south_street():
    ...
    if "Power cell" in items:
        ...
        print_pause("With no power cell to help to see, or weapon to defend "
                    "yourself, you are easily defeated.")
...
def play_again():
    while True:
        ...
        if "yes" in play:
            ...
            print_pause("Sorry, please enter a valid response.")
